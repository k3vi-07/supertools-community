...
async function handleFile(e: Event): Promise<void> {
  ...
  try {
    ...
    const detector = new (window as unknown as { BarcodeDetector: new (opts: { formats: string[] }) => { detect: (img: ImageBitmapSource) => Promise<Array<{ rawValue: string }> } } }).BarcodeDetector({ formats: ['qr_code'] })
    ...
    result.value = ''
    error.value = '识别失败: ' + (err as Error).message + '（请确保图片含清晰二维码）'
  }
}
