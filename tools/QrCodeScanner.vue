...
async function handleFile(e) {
  ...
  try {
    ...
    if (codes.length > 0) {
      result.value = codes.map(function (c) { return c.rawValue }).join('\n')
    } else {
      result.value = ''
      error.value = '未检测到二维码'
    }
...
function copy() {
  window.$he3 && window.$he3.copyText(result.value)
...
.qr-scanner__dropzone:hover { border-color: var(--color-primary); }
...
.qr-scanner__header { display: flex; justify-content: space-between; padding: 8px 12px; border-bottom: 1px solid var(--border-color); font-size: 12px; }
